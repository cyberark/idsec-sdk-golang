...
// Possible IdsecPolicyFilterOperator
const (
	IdsecPolicyFilterOperatorEQ       = "eq"
	IdsecPolicyFilterOperatorContains = "contains"
	IdsecPolicyFilterOperatorOR       = "or"
	IdsecPolicyFilterOperatorAND      = "and"
)

// IdsecPolicyDefaultLimitSize defines the default limit size for policy-access listings.
const (
	IdsecPolicyDefaultLimitSize = 50
)

// filterOperators maps field names to their corresponding filter operators.
var filterOperators = map[string]string{
	"locationType":   IdsecPolicyFilterOperatorEQ,
	"policyType":     IdsecPolicyFilterOperatorEQ,
	"targetCategory": IdsecPolicyFilterOperatorEQ,
	"policyTags":     IdsecPolicyFilterOperatorEQ,
	"status":         IdsecPolicyFilterOperatorEQ,
	"identities":     IdsecPolicyFilterOperatorContains,
}
...
type IdsecPolicyFilters struct {
...
// NewIdsecPolicyFilters initializes a new instance of IdsecPolicyFilters with default values.
func NewIdsecPolicyFilters() *IdsecPolicyFilters {
	return &IdsecPolicyFilters{
		LocationType:         []string{},
...
func (filters *IdsecPolicyFilters) BuildFilterQueryFromFilters() string {
	var clauses []string
	...
	for fieldName, operator := range filterOperators {
		...
		if len(values) > 0 {
			...
			joined := strings.Join(itemClauses, " "+IdsecPolicyFilterOperatorOR+" ")
...
// IdsecPolicyGetAccessPoliciesRequest represents the request to get access policies.
type IdsecPolicyGetAccessPoliciesRequest struct {
	Filters   *IdsecPolicyFilters `json:"filters,omitempty" mapstructure:"filters,omitempty" flag:"filters" desc:"The filter query to apply on the policies"`
...
func (request *IdsecPolicyGetAccessPoliciesRequest) BuildGetQueryParams() IdsecPolicyGetQueryParams {
	queryParams := IdsecPolicyGetQueryParams{
		Limit: request.Limit,
	}
	if queryParams.Limit <= 0 {
		queryParams.Limit = IdsecPolicyDefaultLimitSize
	}
