...
// IdsecUAPSIAVMKeyValTag defines a key/value pair used to match a given tag or label on a VM resource.
type IdsecUAPSIAVMKeyValTag struct {
...
func (t *IdsecUAPSIAVMKeyValTag) Serialize() map[string]interface{} {
...
func (t *IdsecUAPSIAVMKeyValTag) Deserialize(data map[string]interface{}) error {
...
// IdsecUAPSIAVMFQDNRule defines a specific FQDN rule used to match a given DNS record.
type IdsecUAPSIAVMFQDNRule struct {
...
func (r *IdsecUAPSIAVMFQDNRule) Serialize() map[string]interface{} {
	return map[string]interface{}{
		"operator":            r.Operator,
...
func (r *IdsecUAPSIAVMFQDNRule) Deserialize(data map[string]interface{}) error {
...
// IdsecUAPSIAVMIPRule defines a specific logical name rule used to match a given IP+logical name.
type IdsecUAPSIAVMIPRule struct {
...
func (r *IdsecUAPSIAVMIPRule) Serialize() map[string]interface{} {
	return map[string]interface{}{
		"operator":    r.Operator,
...
func (r *IdsecUAPSIAVMIPRule) Deserialize(data map[string]interface{}) error {
	if operator, ok := data["operator"].(string); ok {
		r.Operator = operator
	} else {
		return errors.New("operator must be a string")
...
// IdsecUAPSIAVMAWSResource represents the AWS resources for a virtual machine access policy.
type IdsecUAPSIAVMAWSResource struct {
	Regions    []string                 `json:"regions" mapstructure:"regions" flag:"regions" desc:"The AWS regions where the resources are located."`
	Tags       []IdsecUAPSIAVMKeyValTag `json:"tags" mapstructure:"tags" flag:"tags" desc:"The tags used to match AWS resources. This is a list of key-value pairs."`
...
func (r *IdsecUAPSIAVMAWSResource) Serialize() map[string]interface{} {
...
func (r *IdsecUAPSIAVMAWSResource) Deserialize(data map[string]interface{}) error {
...
// IdsecUAPSIAVMAzureResource represents the Azure resources for a virtual machine access policy.
type IdsecUAPSIAVMAzureResource struct {
	Regions        []string                 `json:"regions" mapstructure:"regions" flag:"regions" desc:"The Azure regions where the resources are located."`
	Tags           []IdsecUAPSIAVMKeyValTag `json:"tags" mapstructure:"tags" flag:"tags" desc:"The tags used to match Azure resources. This is a list of key-value pairs."`
...
func (r *IdsecUAPSIAVMAzureResource) Serialize() map[string]interface{} {
	tags := make([]map[string]interface{}, len(r.Tags))
	for i, tag := range r.Tags {
		tags[i] = tag.Serialize()
	}
...
func (r *IdsecUAPSIAVMAzureResource) Deserialize(data map[string]interface{}) error {
	if regions, ok := data["regions"].([]interface{}); ok {
		for _, region := range regions {
			if str, ok := region.(string); ok {
				r.Regions = append(r.Regions, str)
...
// IdsecUAPSIAVMGCPResource represents the GCP resources for a virtual machine access policy.
type IdsecUAPSIAVMGCPResource struct {
	Regions  []string                 `json:"regions" mapstructure:"regions" flag:"regions" desc:"The GCP regions where the resources are located."`
	Labels   []IdsecUAPSIAVMKeyValTag `json:"labels" mapstructure:"labels" flag:"labels" desc:"The labels used to match GCP resources. This is a list of key-value pairs."`
...
func (r *IdsecUAPSIAVMGCPResource) Serialize() map[string]interface{} {
...
func (r *IdsecUAPSIAVMGCPResource) Deserialize(data map[string]interface{}) error {
	if regions, ok := data["regions"].([]interface{}); ok {
		for _, region := range regions {
			if str, ok := region.(string); ok {
				r.Regions = append(r.Regions, str)
...
// IdsecUAPSIAVMFQDNIPResource represents the fqdn/ip resources for a virtual machine access policy.
type IdsecUAPSIAVMFQDNIPResource struct {
	FQDNRules []IdsecUAPSIAVMFQDNRule `json:"fqdn_rules,omitempty" mapstructure:"fqdn_rules" flag:"fqdn-rules" desc:"The FQDN rules used to match DNS records. This is a list of FQDN rules."`
	IPRules   []IdsecUAPSIAVMIPRule   `json:"ip_rules,omitempty" mapstructure:"ip_rules" flag:"ip-rules" desc:"The IP rules used to match IP addresses and logical names. This is a list of IP rules."`
}

// Serialize converts the fqdn/ip resource to a map.
func (r *IdsecUAPSIAVMFQDNIPResource) Serialize() map[string]interface{} {
...
func (r *IdsecUAPSIAVMFQDNIPResource) Deserialize(data map[string]interface{}) error {
	if fqdnRules, ok := data["fqdn_rules"].([]interface{}); ok {
		for _, rule := range fqdnRules {
			if ruleMap, ok := rule.(map[string]interface{}); ok {
				var fqdnRule IdsecUAPSIAVMFQDNRule
	...
	if ipRules, ok := data["ip_rules"].([]interface{}); ok {
		for _, rule := range ipRules {
			if ruleMap, ok := rule.(map[string]interface{}); ok {
				var ipRule IdsecUAPSIAVMIPRule
...
// IdsecUAPSIAVMPlatformTargets represents the targets for a virtual machine access policy.
type IdsecUAPSIAVMPlatformTargets struct {
...
func (t *IdsecUAPSIAVMPlatformTargets) Serialize(workspace string) (map[string]interface{}, error) {
...
func (t *IdsecUAPSIAVMPlatformTargets) Deserialize(data map[string]interface{}, workspace string) error {
	switch workspace {
	case common.WorkspaceTypeAWS:
		if awsData, ok := data[common.WorkspaceTypeAWS].(map[string]interface{}); ok {
			t.AWSResource = &IdsecUAPSIAVMAWSResource{}
	...
	case common.WorkspaceTypeAzure:
		if azureData, ok := data[common.WorkspaceTypeAzure].(map[string]interface{}); ok {
			t.AzureResource = &IdsecUAPSIAVMAzureResource{}
	...
	case common.WorkspaceTypeGCP:
		if gcpData, ok := data[common.WorkspaceTypeGCP].(map[string]interface{}); ok {
			t.GCPResource = &IdsecUAPSIAVMGCPResource{}
	...
	case common.WorkspaceTypeFQDNIP:
		if fqdnipData, ok := data[common.WorkspaceTypeFQDNIP].(map[string]interface{}); ok {
			t.FQDNIPResource = &IdsecUAPSIAVMFQDNIPResource{}
