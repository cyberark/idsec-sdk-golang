...
type IdsecSIASSOGetTokenInfo struct {
	TokenType string `json:"token_type" validate:"required" mapstructure:"token_type" flag:"token-type" desc:"The token type for which to get the info [DPA-K8S, DPA-DB, DPA-RDP, DPA-SSH]." choices:"password,client_certificate,oracle_wallet,rdp_file"`
	Service   string `json:"service" validate:"required" mapstructure:"service" flag:"service" desc:"The service for which to get the token info [password, client_certificate, oracle_wallet, rdp_file]." choice:"DPA-DB,DPA-K8S,DPA-RDP,DPA-SSH"`
}
