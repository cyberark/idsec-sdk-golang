		...
		},
		{
			...
			mockSecrets: []*vmsecretsmodels.IdsecSIAVMSecret{
				...
				},
				{
					...
					IsActive:      false,
					SecretDetails: `{"account_domain":"domain"}`,
				},
			},
			filter: &vmsecretsmodels.IdsecSIAVMSecretsFilter{
				SecretType: "PCloudAccount",
		...
		},
		{
			...
			mockSecrets: []*vmsecretsmodels.IdsecSIAVMSecret{
				...
				},
				{
					...
					IsActive:      true,
					SecretDetails: `{"account_domain":"MYDOMAIN"}`,
				},
				{
					SecretID:      "secret-3",
					SecretType:    "PCloudAccount",
		...
		},
		{
			...
			mockSecrets: []*vmsecretsmodels.IdsecSIAVMSecret{
				...
				},
				{
					...
					IsActive:      true,
					SecretDetails: `{"account_domain":"OTHERDOMAIN"}`,
				},
			},
			filter: &vmsecretsmodels.IdsecSIAVMSecretsFilter{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, secret := range result {
				if secret.SecretDetails == "" {
					t.Errorf("Secret %s has empty SecretDetails, expected JSON string", secret.SecretID)
				}
			}
		})
	}
