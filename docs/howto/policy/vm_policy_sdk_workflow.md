---
title: VM policy SDK workflow
description: Creating a VM Policy using Idsec SDK
---

# VM policy SDK workflow
Here is an example workflow for adding a VM policy assets via the SDK:
...
- we then configure VM policy

## VM policy with dual control

Here is an example workflow for adding a VM policy with dual control (access approval):
			...
			IdsecPolicyInfraCommonAccessPolicy: policycommomodels.IdsecPolicyInfraCommonAccessPolicy{
				IdsecPolicyCommonAccessPolicy: policycommomodels.IdsecPolicyCommonAccessPolicy{
					Metadata: policycommomodels.IdsecPolicyMetadata{
						...
						Description: "This is an example of a VM access policy for Infrastructure with dual control.",
						Status: &policycommomodels.IdsecPolicyStatus{
							Status: policycommomodels.StatusTypeActive,
						},
						PolicyEntitlement: policycommomodels.IdsecPolicyEntitlement{
...
- The admin user is logged in to perform actions on the tenant
- we then configure a VM policy with dual control (access approval) requiring an approver before access is elevated

Dual control must be enabled for your tenant. If it isn't, `CreatePolicy`/`UpdatePolicy`
return a clear, actionable error explaining that dual control isn't enabled and that you
should either contact Idira support to enable it, or omit `AccessApproval` (leave it
`nil`) to create the policy without dual control.